...
RSpec.describe Game, type: :model do
  ...
  before(:context) do
    ...
    @session = @game2.sessions.create(date: Date.today())
    @player1 = @session.session_players.find_or_create_by(name: 'Player1')
    @player2 = @session.session_players.find_or_create_by(name: 'Player2')
  end
